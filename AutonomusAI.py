...
def run_live(risk_pct: float = RISK_PCT):
    ...
    # Initial Market Briefing
    if handlers:
        first_sym = all_symbols[0]
        brief = analyst.get_market_brief(first_sym, handlers[first_sym].get_ohlc(_TF_H1, count=50))
        log.info(f"AI Assistant Online. {brief}")
            ...
            for sym in list(handlers.keys()):
                ...
                try:
                    ...
                    if signal:
                        ...
                        log.info(f"[{sym}] Assistant is analyzing technical {signal_tag} signal...")
                        approved, confidence, ai_reason = analyst.validate_signal(sym, signal, df_h1, df_m15)
                        
                        if not approved:
                            log.info(f"[{sym}] [FILTERED] Analyst rejected signal: {ai_reason}")
                            continue
                            
                        log.info(f"[{sym}] [APPROVED] {ai_reason} (AI Confidence: {confidence*100:.0f}%)")
